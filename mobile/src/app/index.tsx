...
export default function Index() {
  ...
  function handleRemoveEmail(emailToRemove: string) {
    setEmailsToInvate(prevState => prevState.filter((email) => email !== emailToRemove))
  }

  function handleAddEmail() {
    if(!validateInput.email(emailToInvate)) {
      return Alert.alert("Convidado", "E-mail inválido!")
    }

    const emailAlreadyExists = emailsToInvate.find((email) => email === 
    emailToInvate)
    ...
    setEmailsToInvate(prevState => [...prevState, emailToInvate])
    setEmailToInvate("")
  }
  ...
  return (
    <View className='flex-1 justify-center items-center px-5'>
      ...
      <Image src={require("@/assets/bg.png")} className='absolute'/>
      ...
      <View className='w-full bg-zinc-900 p-4 rounded-lg my-8 border-zinc-800'>
        <Input>
          ...
          editable={stepForm === StepForm.TRIP_DETAILS && true}
        ...
        <Input>
          ...
          <Input.Field 
            ...
            autoCorrect={false}
            value={emailsToInvate.length > 0 ? `${emailsToInvate.length} pessoas(a) convidadas(s)` : ""}
            onPressIn={() => {
              Keyboard.dismiss()
        ...
        <View className='gap-4 mt-4'>
            <Input variant='secondary'>
              ...
              <Input.Field  
                ...
                onChangeText={(text) => setEmailToInvate(text.toLowerCase())}
                value={emailToInvate}
              />
            </Input>
